...
public class NukeDown : MonoBehaviour {
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            GroundFire fire = groundFire.GetComponent<GroundFire>();
            fire.damage = damage;
            Instantiate(fire, new Vector3(transform.position.x, transform.position.y - .05f, 0), Quaternion.identity);
            Destroy(gameObject);
        }
    }
}
