...
public class BombingFlyingEnemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (follow)
        {
            ...
            if (Vector2.Distance(playerPosition, myPosition) < .1f)
            {
                if (!bomb)
                {
                    dropBomb();
    ...
    {
        EnemyAirBomb temp = airBomb.GetComponent<EnemyAirBomb>();
        temp.damage = damage;
        Instantiate(airBomb, new Vector3(transform.position.x, transform.position.y - .1f, 0), Quaternion.identity);
    }
