...
public class Utility1 : MonoBehaviour
{
    ...
    public void FixedUpdate()
    {
        ...
        Debug.Log("this is the value of startTime: " + database.startTime);
        Debug.Log("this is the value of jetpackforce: " + database.JetPackForce);
        if (Input.GetKey(KeyCode.F))
        {
            Debug.Log("do i get into f?");
            database.startTime -= Time.deltaTime;
            rb2d.AddForce(Vector2.up * database.JetPackForce);
        }
        if (database.startTime <= 0.0f)
        {
            //rb2d.velocity = Vector2.down * Time.smoothDeltaTime;
        ...
        Vector2 movement = new Vector2((horizontal * stats.runSpeed * Time.deltaTime) + transform.position.x, 0+transform.position.y);
